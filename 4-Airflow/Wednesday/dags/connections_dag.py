...
# TODO: Import the required hooks
...
def query_database(**context):
    """
    TODO: Query the Airflow metadata database.
    ...
    dag_run_count = result[0] if result else 0
    print(f"Total DAG runs: {dag_run_count}")

    return {"dag_run_count": dag_run_count}
    


def show_connection_info(**context):
    """
    TODO: Display connection information (without the password!).
...
def query_task_instances(**context):
    ...
    print("Recent task instances:")
    for row in results:
        dag_id, task_id, state = row
        print(f"DAG: {dag_id}, Task: {task_id}, State: {state}")
